...
bool wm8960_init_playback(void)
{
    ...
    if (!wm8960_write_reg(WM8960_REG_PWR_MGMT2,  0x160)) return false;
    ...
    if (!wm8960_write_reg(WM8960_REG_ROUT1_VOL, 0x000)) return false;
    ...
    if (!wm8960_write_reg(WM8960_REG_ROUT_MIX,  0x000)) return false;

    ESP_LOGI(TAG, "playback initialized (headphone-only, onboard MCLK, "
             "44.1kHz, 16-bit stereo, right HP disabled)");
    return true;
}
